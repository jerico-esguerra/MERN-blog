...
const SavedPost = ({ post, setCurrentId }) => {
  ...
  const Saved = () => {
    return saved?.find((save) => save === (userId))
        ? (
  ...
  return (
    <Card className={classes.card} raised elevation={6}>
      ...
      >
        ...
        <CardContent>
          <Typography variant="body2" color="textSecondary" component="p">{post.message.split(' ').splice(0, 20).join(' ').substring(0, 20)}...</Typography>
        </CardContent>
      </ButtonBase>
      <CardActions className={classes.cardActions}>
        ...
        {(user?.result?._id || user?.result?.sub ) && (
          <Button size="small" color="primary" disabled={!user?.result} onClick={handleSave}>
            <Saved />
          </Button>
        )}
        </div>
